...
if menu == "Jurnal Umum":
    ...
    with st.expander("Tambah Transaksi"):
        ...
        nama_akun = st.selectbox("Nama Akun", df_akun, key="akun_input")
        keterangan = st.text_input("Keterangan", key="ket_input")
        saldo_debit = st.number_input("Saldo Debit", min_value=0.0, key="debit_input")
        saldo_kredit = st.number_input("Saldo Kredit", min_value=0.0, key="kredit_input")
    ...
    if not df.empty:
        ...
        # Expander hapus per baris dengan tombol per baris
        with st.expander("Hapus Baris Tertentu"):
            st.markdown("Klik tombol 'Hapus' di samping untuk menghapus baris tertentu.")
            
            # Pastikan ada kolom id di df, kalau belum, modifikasi ambil_jurnal di database.py agar id ikut di-select
            for i, row in df.iterrows():
                col1, col2, col3, col4, col5 = st.columns([1, 3, 2, 2, 1])
                col1.write(i)
                col2.write(row["Nama Akun"])
                ...
                if col5.button("Hapus", key=f"hapus_{row.name}"):
                    hapus_jurnal_id(row.name)  # row.name diasumsikan adalah id primary key
                    st.warning(f"Baris dengan akun '{row['Nama Akun']}' telah dihapus.")
                    st.rerun()
